...
#if UNITY_IOS && UNITY_IOS14_SUPPORT
...
namespace GameLift.PrivacyConsent
{
    ...
    {
        public UniTask<bool> RequestConsentAsync()
        {
            Debug.Log("[PrivacyConsentService] RequestConsentAsync started.");

#if UNITY_IOS && UNITY_IOS14_SUPPORT
            return RequestIosConsentAsync();
#elif UNITY_IOS
            Debug.Log("[PrivacyConsentService] iOS ATT support package is not installed. Skipping ATT and returning true.");
...
#if UNITY_IOS && UNITY_IOS14_SUPPORT
        private async UniTask<bool> RequestIosConsentAsync()
        {
